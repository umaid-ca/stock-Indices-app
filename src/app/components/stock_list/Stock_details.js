...
const Stock_details = () => {
  ...
  const [selectedData, useSelectedData ] = useState(null)
  ...
  async function getData() {
    ...
    try {
      ...
      let claculation = profitCalculation(respData.results[0].o, respData.results[0].c)
      let claculationPrice = profitPriceCalculation(respData.results[0].o, respData.results[0].c)
  ...
  useEffect(() => {
    useSelectedData(JSON.parse(localStorage.getItem("Indice")))
  }, [])
  ...
  return (
    <div style={{padding: "0px 20px"}}>
      <button onClick={() => router.push("/")} className="back_btn">{"< Back"}</button>
      <h1>Stock Indice: {selectedData ? selectedData.name : "Now Selected Data"}</h1>
      <h2>You may be interested in: </h2>
      <div className="stock_data">
      <span>{selectedData?.symbol}<span style={{marginLeft: '30px'}}>{selectedData?.name}</span></span>
      <span>${indiceData?.c?.toFixed(2)}</span>
      <span style={textStyle}>{percentagePorL}%</span>
      <span style={textStyle}>${pricePorL}</span>
      </div>
     {indiceData != undefined && <div>
        <LineChart />
      </div>}
      {indiceData == undefined &&  <div>
        <h1>
